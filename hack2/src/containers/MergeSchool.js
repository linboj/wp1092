...
class MergeSchool extends Component {
    ...
    moveLeft = (prevBoard) => {
        ...
        let combination = 0;
        let newres=this.moveRight(board)
        board=this.rotateClockwise(this.rotateClockwise(newres.board))
        return {board, combination};
    };
    ...
    componentDidMount() {
        ...
        body.onkeydown=this.handleKeyDown.bind(this)
    }
    ...
    render() {
        return (
            <>      
                <Header generalRank={this.state.qs_ranking} step={this.state.step} bestRank={this.state.best_qs_ranking} restart={this.setrestart}/>
